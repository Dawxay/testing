...
function getColor(score, theme = "dark") {
  const c = themeColors[theme];

  if (score >= 0.90) return c.lightGreen;
  if (score >= 0.80) return c.green;
  if (score >= 0.70) return c.darkishGreen;
  if (score >= 0.60) return c.darkSaladGreen;
  if (score >= 0.50) return c.yellow;
  if (score >= 0.30) return c.orange;
  if (score >= 0.15) return c.darkOrange;
  if (score >= 0.00) return c.red;
  return c.gray;
}
const bNodes = [
    {
        ...
        "position": { "x": 0, "y": 1000 },
        "data": {
            "info": "get started with (insert app name here)",
...
function compileNodes(nodes) {
    return nodes.map(node => ({
        ...node,
        type: node.id === "0" ? "start" : "circle"
    }));
}
function compileEdges(edges) {
...
function circleND({data}) {
    console.log(data);
    return (
        <div style={{ border: "none", borderRadius: "50%", backgroundColor: getColor(data.choiceScore) }}
            className="node w-12 h-12 border-none cursor-default">
...
function startND() {
    return (
        <div style={{ border: "none", borderRadius: "50%", backgroundColor: "gray" }}
            className="node w-12 h-12 border-none cursor-default">
            <Handle type="source" position={Position.Top} style={{
                border: "none",
                width: "50px",
...
export default function Flow() {
    ...
    const [edges, setEdges, onEdgesChange] = useEdgesState(compileEdges(dfEdges));
    ...
    return (
        ...
        <aside className='w-[80vw] h-[60vw] border-2 border-[#202020] bg-[#131a1a] m-auto'>
            <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onEdgesChange={onEdgesChange} onConnect={connect} proOptions={{ hideAttribution: true }} fitView minZoom={0.1}>
                <Background id='sm' variant='dots' gap={100} color='#FFFFFF' className='bg-white' />
            </ReactFlow>
        </aside>
        <div className="flex flex-col items-center gap-2 mt-6">
  <div style={{ backgroundColor: themeColors.dark.red }} className="w-24 h-10 rounded"></div>
  <div style={{ backgroundColor: themeColors.dark.darkOrange }} className="w-24 h-10 rounded"></div>
  <div style={{ backgroundColor: themeColors.dark.orange }} className="w-24 h-10 rounded"></div>
  <div style={{ backgroundColor: themeColors.dark.yellow }} className="w-24 h-10 rounded"></div>
  <div style={{ backgroundColor: themeColors.dark.darkSaladGreen }} className="w-24 h-10 rounded"></div>
  <div style={{ backgroundColor: themeColors.dark.darkishGreen }} className="w-24 h-10 rounded"></div>
  <div style={{ backgroundColor: themeColors.dark.green }} className="w-24 h-10 rounded"></div>
  <div style={{ backgroundColor: themeColors.dark.lightGreen }} className="w-24 h-10 rounded"></div>
</div>
</>
    )
}
    ...
    //Also all paths wont be shown at all times, instead the user will see a brief overview of all paths, and they would be able to zoom in to see it in more detail. This would make it possible for millions of lines and nodes to exist without React flow dying. also the user will be able to zoom into a day for example and it will show paths of what they can do in that day and then the outcome as well even just for that day and they would be able to zoom out and see more abstract choices and outcomes